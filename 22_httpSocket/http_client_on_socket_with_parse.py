...
class MyHTMLParser(HTMLParser):
    # множество всех тегов
    tags = {}

    # список элементов и их текстов
    texts = []
    ...
    def handle_starttag(self, tag, attrs):
        ...
        self._tag_name = tag
        if tag == 'a':
            href_attr = [attr for attr in attrs if attr[0] == 'href']
            if len(href_attr) == 1:
                self.links.append(href_attr[0][1])
        if tag == 'img':
            src_attr = [attr for attr in attrs if attr[0] == 'src']
            if len(src_attr) == 1:
                self.images.append(src_attr[0][1])

    def handle_endtag(self, tag):
        pass

    def handle_data(self, content):
        self.texts.append((self._tag_name, content))
        pass
...
if __name__ == '__main__':
    ...
    print('====================')
    print('Список всех тегов: ', parser.tags)
    max_k = None
    for k, v in parser.tags.items():
        if max_k is None:
            max_k = k
        else:
            if v > parser.tags[max_k]:
                max_k = k
    print('Самый частовстречающийся тег "{tag}", всего встречается {count}'.format(
        tag=max_k,
        count=parser.tags[max_k]
    ))
    print('Список всех тегов содержащих текст: ', parser.texts)
    print('Список всех ссылок на странице: ', parser.links)
    print('Список всех картинок на странице: ', parser.images)
